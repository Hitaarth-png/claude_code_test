...
<div id="app">
  <div id="sidebar">
    ...
    <fieldset>
      <legend>Shade areas by (pick 1 or 2)</legend>
      <p style="font-size:11px; color:#666; margin:0 0 6px;">Picking a 2nd metric blends both into one colour per area (see legend).</p>
      <label><input type="checkbox" class="metric-cb" value="idaci_decile" checked> IDACI decile (1 = most deprived children)</label>
...
let selectedMetrics = ['idaci_decile'];
...
// their fixed natural scale; continuous metrics use the actual data range.
const METRIC_INFO = {
  idaci_decile: { label: 'IDACI decile', low: 1, high: 10, lowLabel: 'Most deprived', highLabel: 'Least deprived' },
...
// (0=low..2=high), cols = metric A tercile (0=low..2=high).
...
function getTercile(key, value) {
  if (value === null || value === undefined) return null;
  const { low, high } = METRIC_INFO[key];
  const t = high > low ? (value - low) / (high - low) : 0;
  return Math.min(2, Math.max(0, Math.floor(t * 3)));
}
...
function styleFor(feature) {
  ...
  if (selectedMetrics.length === 2) {
    ...
    const ta = getTercile(keyA, p[keyA]);
    const tb = getTercile(keyB, p[keyB]);
    fill = (ta === null || tb === null) ? NO_DATA_COLOUR : BIVARIATE_PALETTE[tb][ta];
  } else {
    fill = singleMetricColour(selectedMetrics[0], p);
  }
...
function renderSingleLegend(el, key) {
  if (key === 'pct_children_0015') {
    el.innerHTML = `
...
function renderBivariateLegend(el, keyA, keyB) {
  const a = METRIC_INFO[keyA];
  const b = METRIC_INFO[keyB];
  let grid = '<div style="display:grid; grid-template-columns:repeat(3,18px); gap:2px; margin:6px 0;">';
  for (let row = 2; row >= 0; row--) {
    for (let col = 0; col < 3; col++) {
      grid += `<div style="width:18px;height:18px;background:${BIVARIATE_PALETTE[row][col]};border:1px solid rgba(0,0,0,0.2);"></div>`;
    }
  }
  grid += '</div>';
  el.innerHTML = `
    <div style="font-size:12px; margin-bottom:4px;"><b>${a.label}</b> &rarr;<br>${a.lowLabel} to ${a.highLabel}</div>
    ${grid}
...
function renderLegend() {
  ...
  if (selectedMetrics.length === 2) {
    renderBivariateLegend(el, selectedMetrics[0], selectedMetrics[1]);
  } else {
    renderSingleLegend(el, selectedMetrics[0]);
  }
}
...
document.querySelectorAll('.metric-cb').forEach(cb => {
  cb.addEventListener('change', e => {
    ...
    if (e.target.checked) {
      ...
      if (selectedMetrics.length > 2) {
        const removed = selectedMetrics.shift();
        const removedCb = document.querySelector(`.metric-cb[value="${removed}"]`);
...
function wardReadinessPopupHtml(p) {
  return `
    ...
    <table>
      ...
      <tr><td class="k">Weighted need (children &times; IDACI multiplier)</td><td>${p.need_weighted}</td></tr>
      <tr><td class="k">Provision (capacity / 1,000 weighted children)</td><td>${p.provision_per_1k_weighted}</td></tr>
      <tr><td class="k">Provision gap (need share &minus; supply share)</td><td>${(p.gap_w * 100).toFixed(1)}pp ${p.gap_w > 0 ? '(under-served)' : '(well-served)'}</td></tr>
